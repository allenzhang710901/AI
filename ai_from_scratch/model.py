...
SUPER_SMART_CUES = ["像你一样智能", "像你一样聪明", "更像你", "像chatgpt", "像gpt"]
EMOTION_CUES = {
...
class SimpleChineseAIAssistant:
    ...
    @staticmethod
    def _is_super_smart_request(text: str) -> bool:
        lowered = text.lower().replace(" ", "")
        if any(c in lowered for c in SUPER_SMART_CUES):
            return True
        return ("智能" in lowered or "聪明" in lowered) and any(k in lowered for k in ["像你", "更你", "一样"]) 
    ...
    def _smart_coach_answer(self, text: str) -> str:
        goal = self._extract_goal(text)
        prefix = f"{self.user_name}，" if self.user_name else ""
        return (
            f"{prefix}我会用更接近强模型的方式回答你，不只解释意思，而是给你可执行决策。\n"
            f"【问题定义】你当前核心目标：{goal}。\n"
            "【关键假设】先假设你资源有限、希望快速看到结果。\n"
            "【两条路径】\n"
            "- 路线A（稳）：先打基础 + 小项目迭代，风险低但见效稍慢。\n"
            "- 路线B（快）：直接做MVP并连续试错，见效快但波动更大。\n"
            "【建议选择】如果你每天<2小时，优先A；如果>=2小时且抗压强，优先B。\n"
            "【今天就做】1) 写下目标用户 2) 列3个功能 3) 先做最小可运行版本。\n"
            "你回我‘选A’或‘选B’，我就给你下一步的详细日程。"
        )
    ...
    def reply(self, user_text: str) -> Tuple[str, Prediction]:
        ...
        if self._is_super_smart_request(normalized_text):
            answer = self._smart_coach_answer(normalized_text)
            pred = Prediction(intent="recommend", confidence=0.94)
